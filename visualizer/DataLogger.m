...
classdef DataLogger < handle
    ...
    methods
        ...
        function obj=DataLogger(node, ROBOT_NUM)
            ...
            for i=1:ROBOT_NUM
                obj.subscriber_pos_ref{i}=...
                    ros2subscriber(node, sprintf('/robot_%d/planned_path', i), 'nav_msgs/Path', @(message)obj.CallBack_pos_ref(message, i));
                obj.subscriber_vel_cnt{i}=...
                    ros2subscriber(node, sprintf('/robot%d/cmd_vel', i), 'geometry_msgs/TwistStamped', @(message)obj.CallBack_vel_cnt(message, i));
                obj.subscriber_odometry{i}=...
                    ros2subscriber(node, sprintf('/robot_%d/odometry', i), 'nav_msgs/Odometry', @(message)obj.CallBack_odometry(message, i));
            end

            obj.subscriber_vel_ref=ros2subscriber(node, '/cmd_vel_list', 'std_msgs/Float64MultiArray', @(message)obj.CallBack_vel_ref(message));
            ...
            obj.record_true=false;
            obj.t_start=[];
        end

        % Data logging of reference positions
        function CallBack_pos_ref(obj, message, i)
            if obj.record_true

                % Essential records
                obj.datalog_pos_ref{i}.t(1, end+1)=toc(obj.t_start);
        ...
        function CallBack_vel_ref(obj, message)
            if obj.record_true
                % Essential records
                for i=1:obj.robot_num
                    obj.datalog_vel_ref{i}.t(1, end+1)=toc(obj.t_start);
        ...
        function CallBack_vel_cnt(obj, message, i)
            ...
            if obj.record_true
                % Essential records
                obj.datalog_vel_cnt{i}.t(1, end+1)=toc(obj.t_start);
        ...
        function CallBack_odometry(obj, message, i)
            if obj.record_true
                % Essential records
                obj.datalog_pos_act{i}.t(1, end+1)=toc(obj.t_start);
                obj.datalog_vel_act{i}.t(1, end+1)=toc(obj.t_start);

                obj.datalog_pos_act{i}.x(1, end+1)=message.pose.pose.position.x;
                obj.datalog_pos_act{i}.y(1, end+1)=message.pose.pose.position.y;
                ...
                obj.datalog_pos_act{i}.theta(1, end+1)=temp(1);

                obj.datalog_vel_act{i}.v(1, end+1)=message.twist.twist.linear.x;
                obj.datalog_vel_act{i}.w(1, end+1)=message.twist.twist.angular.z;

                % Message record
                %obj.raw_odometry_message{i}{1, end+1}=message;

                disp("Odometry information successfully recorded!");
            end
        end
    end
end