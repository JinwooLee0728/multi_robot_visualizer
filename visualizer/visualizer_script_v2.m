...
while cont_while_loop
    for robot_idx=1:ROBOT_NUM
        ...
        if length(obj.datalog_vel_act{robot_idx}.t)>=i_v_act(robot_idx)+SAMPLE_INTV
            ...
            addpoints(h_v_act_lines{robot_idx},...
                obj.datalog_vel_act{robot_idx}.t(i_v_act(robot_idx))-obj.t_start,...
    ...
    if ~isempty(lastMessage) && toc(lastMessage)>=5
...
subplot(4, ROBOT_NUM, 1:4*(ROBOT_NUM-1))
hold on
for i=1:ROBOT_NUM
...
for i=1:ROBOT_NUM
    subplot(4, ROBOT_NUM, 4*(ROBOT_NUM-1)+i)
    hold on
